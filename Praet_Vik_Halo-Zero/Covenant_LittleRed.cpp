...
// static texture data
Texture* Covenant_LittleRed::m_pTexture{ nullptr };
Texture* Covenant_LittleRed::m_pTextureDead{ nullptr };
SoundEffect* Covenant_LittleRed::m_pDyingSound{ nullptr };
int Covenant_LittleRed::instanceCounter{ };

Covenant_LittleRed::Covenant_LittleRed(const Rectf& levelBoundaries, const Point2f& position, float scale) :
	// state data
	m_State					{ State::waiting },

	// weapon data
	m_pWeapon				{ new Needler(levelBoundaries, false, scale) },
	m_ShootingAngle			{ },
	...
	m_MinActionRadius		{ 255.f * m_Scale },
	...
	m_HasDropedItems{ false }
{
	if (m_pTexture == nullptr) {
		m_pTexture = new Texture("Textures/Covenant_LittleRed.png");
	}
	if (m_pTextureDead == nullptr) {
		m_pTextureDead = new Texture("Textures/Covenant_LittleRed_Dead.png");
	}
...
Covenant_LittleRed::~Covenant_LittleRed()
{
	--instanceCounter;
...
void Covenant_LittleRed::Update(float elapsedSec, Avatar*& avatar, std::vector<Marine*>& marines, std::vector<BaseObject*>& objects, Level& level)
{
...
void Covenant_LittleRed::Draw() const
{
	glPushMatrix();
	if (!m_IsLookingLeft) {
...
void Covenant_LittleRed::DrawWeapon() const
{
	m_pWeapon->Draw();
}

Rectf Covenant_LittleRed::GetShape() const
{
	return m_Shape;
}

bool Covenant_LittleRed::GetIsDead() const
{
	return m_IsDead;
}

void Covenant_LittleRed::CheckDead()
{
	if (m_Health <= 0)
	{
		if (!m_IsDead)
...
void Covenant_LittleRed::DecreaseHealth(float hitpoints)
{
	m_Health -= hitpoints;
}

BaseWeapon* Covenant_LittleRed::GetWeapon() const
{
	return m_pWeapon;
}

void Covenant_LittleRed::SetWeapon(BaseWeapon*& weapon)
{
	m_pWeapon = weapon;
}

void Covenant_LittleRed::CleanUp()
{
	Covenant_LittleRed::~Covenant_LittleRed();
}

void Covenant_LittleRed::SetSFXVolume(int volume)
{
	m_pDyingSound->SetVolume(volume);
}

void Covenant_LittleRed::CheckForMove(const Rectf& shape)
{
	Point2f shapeCenter{
...
void Covenant_LittleRed::CheckReachedEdge(const Rectf& shape, Level& level)
{
	Point2f shapeCenter{
		shape.left + shape.width / 2,
		shape.bottom + shape.height / 2,
...
void Covenant_LittleRed::DrawTexture() const
{
...
void Covenant_LittleRed::UpdateTexture(float elapsedSec)
{
...
void Covenant_LittleRed::UpdateState()
{
...
void Covenant_LittleRed::CheckClosestPoint(const Rectf& shape)
{
...
void Covenant_LittleRed::HandleShooting()
{
	float WeaponLength{ 35 };
	float WeaponHeight{ 47 };
...
void Covenant_LittleRed::CalcShootingAngle(const Rectf& shape)
{
...
void Covenant_LittleRed::CHeckForDrops(std::vector<BaseObject*>& objects)
{
	if (m_IsDead and !m_HasDropedItems)
	{
		// 1/3 % chance to drop a grenade
