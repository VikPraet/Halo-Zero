...
Texture* PlasmaPistolBullet::m_pBulletTexture{ nullptr };
int PlasmaPistolBullet::instanceCounter{ };

PlasmaPistolBullet::PlasmaPistolBullet(const Point2f& centerPos, float angle, bool isGoingLeft, float scale) :
	m_BulletDamage		{ 10.5f },
						  ...
						  Point2f{m_Shape.left, m_Shape.bottom + m_Shape.height} }
{
	if (m_pBulletTexture == nullptr) {
		m_pBulletTexture = new Texture("Textures/Bullet_Normal_Plasmapistol.png");
	}
	++instanceCounter;
...
void PlasmaPistolBullet::Draw() const
{
	if (m_pBulletTexture != nullptr)
	{
		Rectf bulletRect{
...
void PlasmaPistolBullet::Update(float elapsedSec)
{
...
Point2f PlasmaPistolBullet::GetCenterPos() const
{
...
Point2f PlasmaPistolBullet::GetLeftMiddlePos() const
{
...
Point2f PlasmaPistolBullet::GetRightMiddlePos() const
{
...
float PlasmaPistolBullet::GetBulletDamage() const
{
	return m_BulletDamage;
}


void PlasmaPistolBullet::CleanUp()
{
	PlasmaPistolBullet::~PlasmaPistolBullet();
}