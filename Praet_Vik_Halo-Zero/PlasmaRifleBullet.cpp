...
Texture* PlasmaRifleBullet::m_pBulletTexture{ nullptr };
int PlasmaRifleBullet::instanceCounter{ };

PlasmaRifleBullet::PlasmaRifleBullet(const Point2f& centerPos, float angle, bool isGoingLeft, float scale) :
	m_BulletDamage{ 9.f },
	m_Shape{ 0.f, 0.f, 22.f, 14.f },
	m_CenterPosition{ },
	m_StartPos{ centerPos },
	m_Angle{ angle },
	m_Velocity{ 315.f, 0.f },
						  ...
						  Point2f{m_Shape.left, m_Shape.bottom + m_Shape.height} }
{
	if (m_pBulletTexture == nullptr) {
		m_pBulletTexture = new Texture("Textures/Bullet_Normal_PlasmaRifle.png");
	}
	++instanceCounter;
...
PlasmaRifleBullet::~PlasmaRifleBullet()
{
	--instanceCounter;
...
void PlasmaRifleBullet::Draw() const
{
	if (m_pBulletTexture != nullptr)
	{
		Rectf bulletRect{
...
void PlasmaRifleBullet::Update(float elapsedSec)
{
...
Point2f PlasmaRifleBullet::GetCenterPos() const
{
...
Point2f PlasmaRifleBullet::GetLeftMiddlePos() const
{
...
Point2f PlasmaRifleBullet::GetRightMiddlePos() const
{
...
float PlasmaRifleBullet::GetBulletDamage() const
{
	return m_BulletDamage;
}


void PlasmaRifleBullet::CleanUp()
{
	PlasmaRifleBullet::~PlasmaRifleBullet();
}