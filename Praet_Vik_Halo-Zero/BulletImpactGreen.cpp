...
Texture* BulletImpactGreen::m_pTexture{ nullptr };
int BulletImpactGreen::instanceCounter{ };

BulletImpactGreen::BulletImpactGreen(const Point2f& position, float scale) :
	m_Position{ position },
	m_Scale{ scale },
	m_IsActive{ true },
	m_hasLoopedAnim{ },
	...
	NR_OF_FRAMES{ 5 }
{
	if (m_pTexture == nullptr) {
		m_pTexture = new Texture("Textures/bullet_impact_plasma.png");
	}
	++instanceCounter;
}

BulletImpactGreen::~BulletImpactGreen()
{
	--instanceCounter;
...
void BulletImpactGreen::Draw() const
{
	if (m_IsActive)
	{
		Rectf srcRect{};
...
void BulletImpactGreen::Update(float elapsedSec)
{
