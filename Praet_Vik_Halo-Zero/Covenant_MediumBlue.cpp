...
// static texture data
Texture* Covenant_MediumBlue::m_pTexture{ nullptr };
Texture* Covenant_MediumBlue::m_pTextureDead{ nullptr };
SoundEffect* Covenant_MediumBlue::m_pDyingSound{ nullptr };
int Covenant_MediumBlue::instanceCounter{ };

Covenant_MediumBlue::Covenant_MediumBlue(const Rectf& levelBoundaries, const Point2f& position, float scale) :
	// state data
	...
	m_HasDropedItems		{ false }
{
	if (m_pTexture == nullptr) {
		m_pTexture = new Texture("Textures/Covenant_MediumBlue.png");
	}
	if (m_pTextureDead == nullptr) {
		m_pTextureDead = new Texture("Textures/Covenant_MediumBlue_Dead.png");
	}
...
Covenant_MediumBlue::~Covenant_MediumBlue()
{
	--instanceCounter;
...
void Covenant_MediumBlue::Update(float elapsedSec, Avatar*& avatar, std::vector<Marine*>& marines, std::vector<BaseObject*>& objects, Level& level)
{
...
void Covenant_MediumBlue::Draw() const
{
	glPushMatrix();
	if (!m_IsLookingLeft) {
...
void Covenant_MediumBlue::DrawWeapon() const
{
	m_pWeapon->Draw();
}

Rectf Covenant_MediumBlue::GetShape() const
{
	return m_Shape;
}

bool Covenant_MediumBlue::GetIsDead() const
{
	return m_IsDead;
}

void Covenant_MediumBlue::CheckDead()
{
	if (m_Health <= 0)
	{
		if (!m_IsDead)
...
void Covenant_MediumBlue::DecreaseHealth(float hitpoints)
{
	m_Health -= hitpoints;
}

BaseWeapon* Covenant_MediumBlue::GetWeapon() const
{
	return m_pWeapon;
}

void Covenant_MediumBlue::SetWeapon(BaseWeapon*& weapon)
{
	m_pWeapon = weapon;
}

void Covenant_MediumBlue::CleanUp()
{
	Covenant_MediumBlue::~Covenant_MediumBlue();
}

void Covenant_MediumBlue::SetSFXVolume(int volume)
{
	m_pDyingSound->SetVolume(volume);
}

void Covenant_MediumBlue::CheckForMove(const Rectf& shape)
{
	Point2f shapeCenter{
...
void Covenant_MediumBlue::CheckReachedEdge(const Rectf& shape, Level& level)
{
	Point2f shapeCenter{
		shape.left + shape.width / 2,
		shape.bottom + shape.height / 2,
...
void Covenant_MediumBlue::DrawTexture() const
{
...
void Covenant_MediumBlue::UpdateTexture(float elapsedSec)
{
...
void Covenant_MediumBlue::UpdateState()
{
...
void Covenant_MediumBlue::CheckClosestPoint(const Rectf& shape)
{
...
void Covenant_MediumBlue::HandleShooting()
{
...
void Covenant_MediumBlue::CalcShootingAngle(const Rectf& shape)
{
...
void Covenant_MediumBlue::CHeckForDrops(std::vector<BaseObject*>& objects)
{
	if (m_IsDead and !m_HasDropedItems)
	{
		// 66% chance to drop a grenade
