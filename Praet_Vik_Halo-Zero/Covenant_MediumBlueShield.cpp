...
// static texture data
Texture* Covenant_MediumBlueShield::m_pTexture{ nullptr };
Texture* Covenant_MediumBlueShield::m_pTextureDead{ nullptr };
SoundEffect* Covenant_MediumBlueShield::m_pDyingSound{ nullptr };
int Covenant_MediumBlueShield::instanceCounter{ };

Covenant_MediumBlueShield::Covenant_MediumBlueShield(const Rectf& levelBoundaries, const Point2f& position, float scale) :
	// state data
	...
	m_IsDead				{ false },
	m_HorizontalSpeed		{ 90 * m_Scale },
	m_Velocity				{ 0.f, 0.f },
	m_Acceleration			{ 0.0f * m_Scale, -1350.f * m_Scale },
	m_MinActionRadius		{ 220.f * m_Scale },
	m_MaxActionRadius		{ 410.f * m_Scale },
	m_ShootingRadius		{ 330.f * m_Scale },
	...
	m_HasDropedItems{ false }
{
	if (m_pTexture == nullptr) {
		m_pTexture = new Texture("Textures/Covenant_MediumBlueShield.png");
	}
	if (m_pTextureDead == nullptr) {
		m_pTextureDead = new Texture("Textures/Covenant_MediumBlueShield_Dead.png");
	}
	if (m_pDyingSound == nullptr) {
		m_pDyingSound = new SoundEffect("Sounds/covShieldDeath.mp3");
	}
	++instanceCounter;
...
Covenant_MediumBlueShield::~Covenant_MediumBlueShield()
{
	--instanceCounter;
...
void Covenant_MediumBlueShield::Update(float elapsedSec, Avatar*& avatar, std::vector<Marine*>& marines, std::vector<BaseObject*>& objects, Level& level)
{
...
void Covenant_MediumBlueShield::Draw() const
{
	glPushMatrix();
	if (!m_IsLookingLeft) {
...
void Covenant_MediumBlueShield::DrawWeapon() const
{
	m_pWeapon->Draw();
}

Rectf Covenant_MediumBlueShield::GetShape() const
{
	return m_Shape;
}

bool Covenant_MediumBlueShield::GetIsDead() const
{
	return m_IsDead;
}

void Covenant_MediumBlueShield::CheckDead()
{
	if (m_Health <= 0)
	{
		if (!m_IsDead)
...
void Covenant_MediumBlueShield::DecreaseHealth(float hitpoints)
{
	m_Health -= hitpoints;
}

BaseWeapon* Covenant_MediumBlueShield::GetWeapon() const
{
	return m_pWeapon;
}

void Covenant_MediumBlueShield::SetWeapon(BaseWeapon*& weapon)
{
	m_pWeapon = weapon;
}

void Covenant_MediumBlueShield::CleanUp()
{
	Covenant_MediumBlueShield::~Covenant_MediumBlueShield();
}

void Covenant_MediumBlueShield::SetSFXVolume(int volume)
{
	m_pDyingSound->SetVolume(volume);
}

void Covenant_MediumBlueShield::CheckForMove(const Rectf& shape)
{
	Point2f shapeCenter{
...
void Covenant_MediumBlueShield::CheckReachedEdge(const Rectf& shape, Level& level)
{
	Point2f shapeCenter{
		shape.left + shape.width / 2,
		shape.bottom + shape.height / 2,
...
void Covenant_MediumBlueShield::DrawTexture() const
{
...
void Covenant_MediumBlueShield::UpdateTexture(float elapsedSec)
{
...
void Covenant_MediumBlueShield::UpdateState()
{
...
void Covenant_MediumBlueShield::CheckClosestPoint(const Rectf& shape)
{
...
void Covenant_MediumBlueShield::HandleShooting()
{
	float WeaponLength{ 30 };
	float WeaponHeight{ 52 };
...
void Covenant_MediumBlueShield::CalcShootingAngle(const Rectf& shape)
{
...
void Covenant_MediumBlueShield::CHeckForDrops(std::vector<BaseObject*>& objects)
{
