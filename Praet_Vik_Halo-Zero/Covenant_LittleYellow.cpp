...
// static texture data
Texture* Covenant_LittleYellow::m_pTexture{ nullptr };
Texture* Covenant_LittleYellow::m_pTextureDead{ nullptr };
SoundEffect* Covenant_LittleYellow::m_pDyingSound{ nullptr };
int Covenant_LittleYellow::instanceCounter{ };

Covenant_LittleYellow::Covenant_LittleYellow(const Rectf& levelBoundaries, const Point2f& position, float scale):
	// state data
	m_State					{ State::waiting },

	// weapon data
	m_pWeapon				{ new PlasmaPistol(levelBoundaries, false, scale) },
	m_ShootingAngle			{ },
	...
	// enemy data
	m_Scale					{ scale },
	m_Shape					{ position.x, position.y , 45.f, 75.f},
	m_LevelCollisionShape	{ },
	m_IsDead				{ false },
	m_HorizontalSpeed		{ 100 * m_Scale},
	m_Velocity				{ 0.f, 0.f },
	m_Acceleration			{ 0.0f * m_Scale, -1350.f * m_Scale},
	m_MinActionRadius		{ 250.f * m_Scale },
	m_MaxActionRadius		{ 480.f * m_Scale },
	m_ShootingRadius		{ 350.f * m_Scale },
	...
	m_HasDropedItems		{ false }
{
	if (m_pTexture == nullptr) {
		m_pTexture = new Texture("Textures/Covenant_LittleYellow.png");
	}
	if (m_pTextureDead == nullptr) {
		m_pTextureDead = new Texture("Textures/Covenant_LittleYellow_Dead.png");
	}
	if (m_pDyingSound == nullptr) {
		m_pDyingSound = new SoundEffect("Sounds/covLittleDeath.mp3");
	}
	++instanceCounter;
	...
	m_LevelCollisionShape.width		= 100 * m_Scale;
...
Covenant_LittleYellow::~Covenant_LittleYellow()
{
	--instanceCounter;
...
void Covenant_LittleYellow::Update(float elapsedSec, Avatar*& avatar, std::vector<Marine*>& marines, std::vector<BaseObject*>& objects, Level& level)
{
...
void Covenant_LittleYellow::Draw() const
{
	glPushMatrix();
	if (!m_IsLookingLeft) {
...
void Covenant_LittleYellow::DrawWeapon() const
{
	m_pWeapon->Draw();
}

Rectf Covenant_LittleYellow::GetShape() const
{
	return m_Shape;
}

bool Covenant_LittleYellow::GetIsDead() const
{
	return m_IsDead;
}

void Covenant_LittleYellow::CheckDead()
{
	if (m_Health <= 0)
	{
		if (!m_IsDead)
...
void Covenant_LittleYellow::DecreaseHealth(float hitpoints)
{
	m_Health -= hitpoints;
}

BaseWeapon* Covenant_LittleYellow::GetWeapon() const
{
	return m_pWeapon;
}

void Covenant_LittleYellow::SetWeapon(BaseWeapon*& weapon)
{
	m_pWeapon = weapon;
}

void Covenant_LittleYellow::CleanUp()
{
	Covenant_LittleYellow::~Covenant_LittleYellow();
}

void Covenant_LittleYellow::SetSFXVolume(int volume)
{
	m_pDyingSound->SetVolume(volume);
}

void Covenant_LittleYellow::CheckForMove(const Rectf& shape)
{
	Point2f shapeCenter{
	...
	float distanceToAvatar{ abs(shapeCenter.x - (m_Shape.left + m_Shape.width / 2)) };
...
void Covenant_LittleYellow::CheckReachedEdge(const Rectf& shape, Level& level)
{
	Point2f shapeCenter{
		shape.left + shape.width / 2,
		shape.bottom + shape.height / 2,
...
void Covenant_LittleYellow::DrawTexture() const
{
...
void Covenant_LittleYellow::UpdateTexture(float elapsedSec)
{
...
void Covenant_LittleYellow::UpdateState()
{
...
void Covenant_LittleYellow::CheckClosestPoint(const Rectf& shape)
{
...
void Covenant_LittleYellow::HandleShooting()
{
...
void Covenant_LittleYellow::CalcShootingAngle(const Rectf& shape)
{
...
void Covenant_LittleYellow::CHeckForDrops(std::vector<BaseObject*>& objects)
{
	if (m_IsDead and !m_HasDropedItems)
	{
		// 50% chance to drop a grenade
		int randomNumber{ (rand() % 2) + 1 };
		switch (randomNumber)
		{
		case 1:
			objects.push_back(new PlasmaGrenadeObject(Point2f{ m_Shape.left, m_Shape.bottom }, m_Scale));
			break;
